...
import { ClassKeys, Client, PacketClass, Packets, ResponseType } from "hagcp-network-client";
...
].forEach((name: ClassKeys) => {
  ipcMain.handle(name, (_, data) =>
    client.sendPacketAsync(name, data));
});

ipcMain.on("join_war_request", async (_, data) => {
  const result: Packets.join_war_response = await client.sendClassAsync(PacketClass.join_war_request, data);
  await unsubscribeClient();
  if (result.msg == ResponseType.ok) app.exit();
  console.log(result);
  app.exit(1);
});
...
async function subscribeClient() {
  ...
  await client.sendClassAsync(PacketClass.subscribecommandnodeview);
}

async function unsubscribeClient() {
  await client.sendClassAsync(PacketClass.unsubscribewarmapview);
  await client.sendClassAsync(PacketClass.unsubscribecommandnodeview);
}
...
function attachToClient(webContents: Electron.WebContents) {
  client.once("loggedin", async () => {
    ...
    webContents.send("LoginQueueUpdate", pos.positionInQueue);
  }).on("close", () => {
